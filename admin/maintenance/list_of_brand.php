...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['brand_name'])) {
    ...
    if (isset($_FILES['logo']) && $_FILES['logo']['error'] == UPLOAD_ERR_OK) {
        ...
        echo "<script>alert('Error uploading file: " . $_FILES['logo']['error'] . "');</script>";
    }

   
    $query = "INSERT INTO brand_list (brand_name, status, logo) VALUES (?, ?, ?)";
    $stmt = $connection->prepare($query);
    $stmt->bind_param("sss", $brand_name, $status, $logo);
...
if (isset($_POST['search'])) {
    $search = $_POST['search'];
    $search_query = "%{$search}%";
    $query = "SELECT id, brand_name, date_added, status, logo FROM brand_list WHERE brand_name LIKE ? ORDER BY date_added DESC";
    $stmt = $connection->prepare($query);
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            echo "
                <tr class='border-b border-gray-200 hover:bg-gray-100'>
                    <td class='py-2 px-3'>{$counter}</td> <!-- Use the counter here -->
                        ...
                        <td class='py-2 px-3'>
                             ";
                             if ($row['logo']) {
                                 echo "<img src='uploads/{$row['logo']}' alt='Logo' class='w-10 h-10 object-cover border border-gray-300 rounded-md p-2'>";
                             } else {
                                 echo "No Logo";
                             }
                             echo "
                        </td>
                        <td class='py-2 px-3'>{$row['brand_name']}</td>
                        ...
                        <td class='py-2 px-3 text-center space-x-1'>
                              <a href='#' onclick='openEditForm({$row['id']}, \"{$row['brand_name']}\", \"{$row['status']}\", \"{$row['logo']}\")' class='bg-blue-500 text-white px-2 py-1 rounded hover:bg-blue-600 text-xs'>Edit</a>
                             <a href='delete_brand.php?id={$row['id']}' onclick='return confirm(\"Are you sure?\")' class='bg-red-500 text-white px-2 py-1 rounded hover:bg-red-600 text-xs'>Delete</a>
                        </td>
                    </tr>
                 ";
                 $counter++;
                ...
                echo "
                    <tr>
                        <td colspan='5' class='text-center text-gray-600'>No Brand found</td>
                    </tr>
                ";
            }
...
<body class="relative">
    ...
    <ul class="flex items-center w-full justify-end">
        <li class="list-none inline-block dropdown">
            ...
            <div class="p-4 w-full h-full mt-3">
                <div class="flex flex-col p-2 border-t-4 border-sky-900 rounded-md shadow-2xl bg-white">
                    <div class="flex items-center justify-between mb-4">
                        <h1 class="text-black text-xl">List of Brand</h1>
                            ...
                            <table class='min-w-full bg-white border border-gray-300 rounded-md shadow-sm'>
                                <thead>
                                    <tr class='bg-gray-200 text-gray-700 text-xs uppercase'>
                                        ...
                                        <th class='py-2 px-3 text-left'>Brand Name</th>
                                        <th class='py-2 px-3 text-left'>Status</th>
                                        <th class='py-2 px-3 text-center'>Actions</th>
                                    </tr>
                                </thead>
                                <tbody id='brand-table-body' class='text-gray-600 text-xs'>
                            ";
                    ...
                    <script>
                        function searchCategories() {
                            ...
                            xhr.open('POST', 'list_of_brand.php', true);
            ...
            <!-- Add Brand Modal -->
            <div id="addBrandModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                <div class="bg-white p-6 rounded-md w-1/3">
                    <h2 class="text-lg font-bold mb-4">Add New Brand</h2>
                    <form id="addBrandForm" method="POST" action="list_of_brand.php" enctype="multipart/form-data">
                        <div class="mb-4">
                            <label for="brand_name" class="block text-sm font-medium text-gray-700">Brand Name</label>
                            <input type="text" name="brand_name" id="brand_name" required class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-400">
                        </div>
                        <div class="mb-4">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="file" name="logo" id="logo" class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-400">
                        </div>
            ...
            <!--edit form-->
    <!-- Edit Brand Modal -->
<div id="editBrandModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-white p-6 rounded-md w-1/3">
        <h2 class="text-lg font-bold mb-4">Edit Brand</h2>
        <form id="editBrandForm" method="POST" action="update_brand.php" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="brand_name" id="edit_brand_name" required class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-400">
            </div>
            <div class="mb-4">
            ...
            <div class="mb-4">
                ...
                <input type="file" name="logo" id="edit_logo" class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-400">
...
<script>
    function openEditForm(id, brandName, status, logo) {
